...
LoxClass::LoxClass(const std::string &name,
    ...
    : LoxCallable(LoxObject::CLASS), name(name), superclass(superclass),
      methods(methods) {}

std::shared_ptr<LoxObject>
LoxClass::call(Interpreter *interpreter,
...
std::string LoxClass::getName() const { return name; }
...
std::string LoxClass::str() const { return "<class " + name + ">"; }
