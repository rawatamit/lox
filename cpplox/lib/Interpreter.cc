...
void Interpreter::checkNumberOperand(Token tok,
                                     std::shared_ptr<LoxObject> expr) {
  if (expr->getType() != LoxObject::DOUBLE) {
    throw RuntimeException(tok, "Unary operand must be a number.");
  }
}

void Interpreter::checkNumberOperand(Token tok, std::shared_ptr<LoxObject> exp0,
                                     std::shared_ptr<LoxObject> exp1) {
  if (exp0->getType() != LoxObject::DOUBLE or
      exp1->getType() != LoxObject::DOUBLE) {
    throw RuntimeException(tok, "Binary operands must both be numbers.");
  }
}

std::shared_ptr<LoxObject>
Interpreter::lookupVariable(const Token &name, std::shared_ptr<Expr> expr) {
...
std::shared_ptr<LoxObject>
Interpreter::executeBlock(std::vector<std::shared_ptr<Stmt>> &stmts,
...
std::shared_ptr<LoxObject>
Interpreter::visitClass(std::shared_ptr<Class> klass) {
  ...
  for (std::shared_ptr<Function> method : klass->methods) {
    auto fn = std::make_shared<LoxFunction>(method, env,
                                            method->name.lexeme == "init");
    methods[method->name.lexeme] = fn;
  }

  auto loxklass = std::make_shared<LoxClass>(
      klass->name.lexeme, std::static_pointer_cast<LoxClass>(superclass),
      methods);
...
std::shared_ptr<LoxObject>
Interpreter::visitFunction(std::shared_ptr<Function> stmt) {
...
std::shared_ptr<LoxObject>
Interpreter::visitExpression(std::shared_ptr<Expression> stmt) {
  evaluate(stmt->expr);
  return nullptr;
}
...
std::shared_ptr<LoxObject>
Interpreter::visitPrint(std::shared_ptr<Print> stmt) {
...
std::shared_ptr<LoxObject>
Interpreter::visitWhile(std::shared_ptr<While> stmt) {
...
std::shared_ptr<LoxObject>
Interpreter::visitReturn(std::shared_ptr<Return> stmt) {
  std::shared_ptr<LoxObject> value = std::make_shared<LoxNil>();
...
std::shared_ptr<LoxObject>
Interpreter::visitBlock(std::shared_ptr<Block> stmt) {
  return executeBlock(stmt->stmts, std::make_shared<Environment>(env));
}

std::shared_ptr<LoxObject>
Interpreter::visitLogical(std::shared_ptr<Logical> expr) {
...
std::shared_ptr<LoxObject>
Interpreter::visitAssign(std::shared_ptr<Assign> expr) {
...
std::shared_ptr<LoxObject>
Interpreter::visitBinaryExpr(std::shared_ptr<BinaryExpr> expr) {
  ...
  case TokenType::SLASH:
    ...
    return std::make_shared<LoxDouble>(
        ...
        std::dynamic_pointer_cast<LoxDouble>(rhs)->getValue());
  case TokenType::PLUS:
    if (lhs->getType() == LoxObject::DOUBLE and
        rhs->getType() == LoxObject::DOUBLE) {
      return std::make_shared<LoxDouble>(
          std::dynamic_pointer_cast<LoxDouble>(lhs)->getValue() +
          std::dynamic_pointer_cast<LoxDouble>(rhs)->getValue());
    } else if (lhs->getType() == LoxObject::STRING and
               rhs->getType() == LoxObject::STRING) {
    ...
    throw RuntimeException(
        expr->Operator, "Binary operands must be two numbers or two strings.");
...
std::shared_ptr<LoxObject> Interpreter::visitGet(std::shared_ptr<Get> expr) {
  ...
  if (object->getType() == LoxObject::INSTANCE) {
    return std::static_pointer_cast<LoxInstance>(object)->get(expr->name);
  }
...
std::shared_ptr<LoxObject> Interpreter::visitSet(std::shared_ptr<Set> expr) {
  ...
  if (object->getType() == LoxObject::INSTANCE) {
    std::shared_ptr<LoxObject> v = evaluate(expr->value);
    std::static_pointer_cast<LoxInstance>(object)->set(expr->name, v);
    return v;
  }
...
std::shared_ptr<LoxObject> Interpreter::visitThis(std::shared_ptr<This> expr) {
  return lookupVariable(expr->keyword, std::dynamic_pointer_cast<Expr>(expr));
}

std::shared_ptr<LoxObject>
Interpreter::visitSuper(std::shared_ptr<Super> expr) {
  int distance = locals.find(expr)->second;

  std::shared_ptr<LoxClass> superclass =
      std::static_pointer_cast<LoxClass>(env->get(distance, "super"));

  std::shared_ptr<LoxInstance> object =
      std::static_pointer_cast<LoxInstance>(env->get(distance - 1, "this"));

  std::shared_ptr<LoxFunction> method = std::static_pointer_cast<LoxFunction>(
...
std::shared_ptr<LoxObject>
Interpreter::visitGroupingExpr(std::shared_ptr<GroupingExpr> expr) {
  return evaluate(expr->expression);
}

std::shared_ptr<LoxObject>
Interpreter::visitLiteralExpr(std::shared_ptr<LiteralExpr> expr) {
  switch (expr->type) {
...
std::shared_ptr<LoxObject>
Interpreter::visitUnaryExpr(std::shared_ptr<UnaryExpr> expr) {
...
std::shared_ptr<LoxObject>
Interpreter::visitVariable(std::shared_ptr<Variable> expr) {
  return lookupVariable(expr->name, std::static_pointer_cast<Expr>(expr));
}
