...
public:
  LoxFunction(std::shared_ptr<Function> declaration,
      ...
      : LoxCallable(LoxObject::FUNCTION), declaration(declaration),
  ...
  call(Interpreter *interpreter,
       std::vector<std::shared_ptr<LoxObject>> &args) override {
    auto env = std::make_shared<Environment>(closure);

    for (std::vector<Token>::size_type i = 0; i < declaration->params.size();
         ++i) {
  ...
  virtual unsigned arity() const override { return declaration->params.size(); }
